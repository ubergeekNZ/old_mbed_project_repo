...
average_array = np.zeros((8, 8), dtype=float)
...
therm_plot = ax_lst[0].imshow(average_array, interpolation='nearest',
                            vmin=thermo_vmin,
                            vmax=thermo_vmax,
                            cmap='YlOrRd')
                    ...
                    for track_idx, track_item in enumerate(self.current_track_object_list):
                        ...
                        if abs(found_item.pixel_x - track_item.pixel_x) <= 1 and \
                           abs(found_item.pixel_y - track_item.pixel_y) <= 2 :
                            ...
                            if ((track_item.start_pixel_y == 1 or track_item.start_pixel_y == 0) \
                                and track_item.increment == False):
                                if (track_item.pixel_y == 2 or track_item.pixel_y == 3) \
                                    and (found_item.pixel_y == 4 or found_item.pixel_y == 5):
                                    up_counter += 1
                                    track_item.increment = True
                            ...
                            if ((track_item.start_pixel_y == 7 or track_item.start_pixel_y == 6)\
                                and track_item.decrement == False):
                                if (track_item.pixel_y == 5 or track_item.pixel_y == 4) \
                                 and (found_item.pixel_y == 3 or found_item.pixel_y == 2):
                                    # print "<<<< down ++ >>>>"
                                    down_counter = down_counter + 1
                                    track_item.decrement = True
    ...
    def find_objects(self):
        ...
        for j in range(columns):
            for i in range(rows):
                ...
                diff = abs(self.frame[i][j] - self.average)
                if diff >= 1.5:
                    ts = time.time()
                    print str(ts) + ": Found peak at: " + str(diff) + "-" + str(i) + "," + str(j)
                    # Add peak to the found object list to be used by the tracker
...
class ProcessFrame():
    ...
    def process_data(self, rxdata):
        ...
        if "T" in rxdata:
            ...
            # Need to perform a moving average filter on the pixels
